...
export function FirebaseInitializer({ children }: { children: React.ReactNode }) {
  ...
  const initializeFirebase = async () => {
    ...
    try {
      ...
      console.warn("Firebase connection failed, falling back to mock data:", err)
      // Don't show error for demo mode - just continue with mock data
      setIsInitialized(true)
      setError(null)
    } finally {
      setIsRetrying(false)
  ...
  useEffect(() => {
    // If Firebase is not available, go directly to demo mode
    if (!db) {
      console.log("Firebase not configured - using demo mode with sample data")
      setIsInitialized(true)
      return
    }
