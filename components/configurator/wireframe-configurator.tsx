...
import { ChevronLeft, ChevronRight, ShoppingCart, Maximize2, RotateCcw, Check } from "lucide-react"
...
export function WireframeConfigurator({
        ...
        return (
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                {JACKET_STEPS[4]?.sizes?.map((size) => (
                  <div
                    key={size.id}
                    onClick={() => updateState({ size: size.id })}
                    className={`
                      p-3 rounded-lg border-2 cursor-pointer transition-all
                      ${
                        configuratorState.size === size.id
                          ? "border-blue-500 bg-blue-50"
                          : "border-gray-200 hover:border-gray-300"
                      }
                    `}
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-xs text-gray-600">{size.measurements}</div>
                      </div>
                      {size.price > 0 && <span className="text-green-600 font-semibold text-sm">+${size.price}</span>}
                    </div>
                  </div>
                ))}
              </div>
  ...
  const renderPantsStepContent = () => {
    // Similar structure for pants - implement based on PANTS_STEPS
    return <div>Pants customization content for step {currentStep}</div>
  }

  const renderShirtStepContent = () => {
    // Existing shirt customization logic
